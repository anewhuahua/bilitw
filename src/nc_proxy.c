...
static rstatus_t
proxy_reuse_port(struct conn *p)
{
    rstatus_t status;
...
rstatus_t
proxy_find_env_listen(void *elem, void *data)
{
    struct server_pool *sp = data;
	struct server_pool *ep = elem;
	if (string_compare(&sp->name, &ep->name)==0) {
		// magic happens here, ugly, refac
		return ep->dist_type;  
	}
    return NC_OK;
}
...
proxy_listen(struct context *ctx, struct conn *p)
{
    ...
    ASSERT(p->proxy);
	// magic happens here, ugly, refac
    // p->sd = array_each(&env_global.pool, proxy_find_env_listen, pool);
...
#ifdef GRACEFUL	
	tp = array_push(&pool->ctx->listen_conns);
	*tp = p;
#endif
...
#ifdef GRACEFUL
	status = array_init(&ctx->listen_conns, npool, sizeof(struct conn*));
    if (status != NC_OK) {
        return status;
    }
#endif
...
#ifdef GRACEFUL
		while (array_n(&ctx->listen_conns)) {
			array_pop(&ctx->listen_conns);
			// free memory;	
		}
#endif
...
proxy_accept(struct context *ctx, struct conn *p)
{
    ...
    for (;;) {
		...
		//log_debug(LOG_NOTICE, "accepted conn %d", nc_worker_index);
