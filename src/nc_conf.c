...
rstatus_t
conf_pool_master(void *elem, void *data)
{
	rstatus_t status;
	...
	family = cp->listen.info.family;
	addrlen = cp->listen.info.addrlen;
	addr = (struct sockaddr *)&cp->listen.info.addr;

	log_error("family:%d, backlog: %d", family,  cp->backlog);
	
	// !!!!!
	// workaround, dist_type is used in master env, for listen proxy fd.
	// !!!!
	//sp->dist_type = tw_master_listen(family, addr, addrlen, cp->backlog);	
