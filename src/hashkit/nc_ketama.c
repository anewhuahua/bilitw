...
ketama_update(struct server_pool *pool)
{
    ...
    continuum_addition = KETAMA_CONTINUUM_ADDITION;	//10
    points_per_server = KETAMA_POINTS_PER_SERVER;	// 160
    /*
    ...
    if (nlive_server > pool->nserver_continuum) {
        ...
        uint32_t nserver_continuum = nlive_server + continuum_addition; // 10 + 6
        uint32_t ncontinuum = nserver_continuum * points_per_server; // (10 + 6) * 160
        ...
        pool->continuum = continuum;	//pointer
        pool->nserver_continuum = nserver_continuum;   // 10 + 6
    ...
    for (server_index = 0; server_index < nserver; server_index++) {
        ...
        pointer_per_hash = 4;					 // 1/5 * 160 / 4 * 5 * 4
        ...
        for (pointer_index = 1;
            ...
            for (x = 0; x < pointer_per_hash; x++) {	// 2 for , 160
